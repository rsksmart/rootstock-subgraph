  ...
  if (liquidityPool === null) {
    ...
    if (type == 1) {
      ...
      if (!isFeeSplit) {
        ...
        const converterContract = LiquidityPoolV1ConverterContract.bind(converterAddress)
        ...
        const converterMaxConversionFeeResult = converterContract.try_maxConversionFee()
        if (!converterMaxConversionFeeResult.reverted) {
          liquidityPool.maxConversionFee = converterMaxConversionFeeResult.value
        }
        const conversionFee = converterContract.try_conversionFee()
        ...
        liquidityPool.type = 1
        const converterContract = LiquidityPoolV1ConverterContract_V2.bind(converterAddress)
        const converterOwnerResult = converterContract.try_owner()
        if (!converterOwnerResult.reverted) {
          liquidityPool.owner = converterOwnerResult.value.toHex()
        }
...
function isFeeSplitAbi(address: Address): boolean {
  const converterContract = LiquidityPoolV1ConverterContract_V2.bind(address)
  const protocolFeeTokensHeldResult = converterContract.try_protocolFeeTokensHeld(Address.fromString(WRBTCAddress))
...
function getPoolType(address: Address): number {
  const converterContract = LiquidityPoolV1ConverterContract.bind(address)
