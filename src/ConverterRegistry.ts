...
export function handleConverterAnchorAdded(
  ...
  let entity = new ConverterAnchorAdded(
...
export function handleConverterAnchorRemoved(
  event: ConverterAnchorRemovedEvent
): void {
  let entity = new ConverterAnchorRemoved(
    event.transaction.hash.toHex() + "-" + event.logIndex.toString()
  )
  entity._anchor = event.params._anchor
  let transaction = loadTransaction(event)
...
export function handleConvertibleTokenAdded(
  event: ConvertibleTokenAddedEvent
): void {
  let entity = new ConvertibleTokenAdded(
    event.transaction.hash.toHex() + "-" + event.logIndex.toString()
  )
...
export function handleConvertibleTokenRemoved(
  event: ConvertibleTokenRemovedEvent
): void {
  let entity = new ConvertibleTokenRemoved(
    event.transaction.hash.toHex() + "-" + event.logIndex.toString()
  )
  entity._convertibleToken = event.params._convertibleToken
  entity._smartToken = event.params._smartToken
...
export function handleSmartTokenAdded(event: SmartTokenAddedEvent): void {
  let entity = new SmartTokenAdded(
    event.transaction.hash.toHex() + "-" + event.logIndex.toString()
  )
  entity._smartToken = event.params._smartToken
  let transaction = loadTransaction(event)
...
export function handleSmartTokenRemoved(event: SmartTokenRemovedEvent): void {
  let entity = new SmartTokenRemoved(
    event.transaction.hash.toHex() + "-" + event.logIndex.toString()
  )
  entity._smartToken = event.params._smartToken
  let transaction = loadTransaction(event)
